...
def render_data_list_page(analyzer: OrderAnalyzer):
    ...
    st.info(f"📊 주문 **{total_orders:,}건** / 취소 **{total_cancel}건**  |  💰 **{total_revenue:,}원**")

    for i, p in enumerate(periods):
        loaded_at = p.get('로드일시', '')
        fname = p.get('파일명', '')
        ...
        with col1:
            st.markdown(f"**📅 {p['기간']}**")
            if loaded_at:
                label = "자동수집" if fname == 'auto-collected' else "수동입력"
                st.caption(f"입력: {loaded_at} ({label})")
        with col2:
